...
void connectEmptyChannels(Router* router, int routerId, std::vector<RouterChannel*>* inputChannels,
    ...
    if (isNorthDisconnected) {
        ...
        outputChannel = new RouterChannel(
            std::string(channelBaseName + std::to_string(Link::North) + "_Output").c_str());
    ...
    if (isSouthDisconnected) {
        ...
        outputChannel = new RouterChannel(
            std::string(channelBaseName + std::to_string(Link::South) + "_Output").c_str());
    ...
    if (isEastDisconnected) {
        ...
        outputChannel = new RouterChannel(
            std::string(channelBaseName + std::to_string(Link::East) + "_Output").c_str());
    ...
    if (isWestDisconnected) {
        ...
        outputChannel = new RouterChannel(
            std::string(channelBaseName + std::to_string(Link::West) + "_Output").c_str());
...
void assembleNoC(const std::vector<Router*>& routers, std::vector<RouterChannel*>& routerInputChannels,
    ...
    for (unsigned i = 1; i < NOC_SIZE; i++) {
        ...
        if (ceil(float(i) / NOC_ROW_SIZE) == ceil(float(i + 1) / NOC_ROW_SIZE)) {
            ...
            NoCDebug::printDebug(
                std::string("> Connect R" + std::to_string(sourceA) + " to R" + std::to_string(i) + " (West to East)"),
                NoCDebug::Assembly);
        }
        ...
        if (NOC_SIZE != NOC_ROW_SIZE && i >= NOC_ROW_SIZE) {
            ...
            RouterChannel* inputChannel = new RouterChannel(std::string(routerChannelName + "_Input").c_str());
            RouterChannel* outputChannel = new RouterChannel(std::string(routerChannelName + "_Output").c_str());
            // 将同一行上的两个路由器通过输出输出通道连接, 南北纵向连接
            connectRouters(*routers.at(sourceB), *routers.at(i), *inputChannel, *outputChannel, false);
            routerInputChannels.push_back(inputChannel);
            routerOutputChannels.push_back(outputChannel);
            NoCDebug::printDebug(std::string("> Connect R" + std::to_string(sourceB) + " to R" + std::to_string(i) +
                                             " (North to South)"),
                                 NoCDebug::Assembly);
        }
    }
...
void connectStrayChannels(std::vector<Router*>& routers, std::vector<RouterChannel*>& routerInputChannels,
    ...
    for (int i = 0; i < NOC_SIZE; i++) {
        ...
        RouterChannel* inputChannel = new RouterChannel(
            std::string("RouterChannel_" + std::to_string(i) + "_" + std::to_string(i) + "_Input").c_str());
        RouterChannel* outputChannel = new RouterChannel(
            std::string("RouterChannel_" + std::to_string(i) + "_" + std::to_string(i) + "_Output").c_str());
        ...
        if (!ni->getFrontEndReference()) {
            ProcessorElementNull* nullPE = new ProcessorElementNull(
                std::string("NullPE_" + std::to_string(processorElementsNull.size())).c_str(), i);
            ProcessorElementNullShell* nullPEShell = new ProcessorElementNullShell(
                std::string("NullPEShell_" + std::to_string(processorElementNullShells.size())).c_str(), i);
...
void connectMastersAndSlaves(const std::vector<NetworkInterface*>& networkInterfaces,
                             ...
                             const std::vector<int>& masterPositions, const std::vector<int>& slavePositions,
                             int numberOfPairs, char* initialSlaveChar) {
    for (int i = 0; i < numberOfPairs; i++) {
        ProcessorElementMaster* master = new ProcessorElementMaster(std::string("Master_" + std::to_string(i)).c_str(),
                                                                    masterPositions.at(i), slavePositions.at(i));
        ProcessorElementMasterShell* masterShell = new ProcessorElementMasterShell(
            std::string("MasterShell_" + std::to_string(i)).c_str(), masterPositions.at(i), slavePositions.at(i));
        sc_fifo<int>* fifoMasterInt = new sc_fifo<int>();
        sc_fifo<char>* fifoMasterChar = new sc_fifo<char>();

        // 将Msater和MsterShell的输入输出接口连接起来了
        ...
        // 将MsaterShell连接到NetworkInterface的后端
        ...
        ProcessorElementSlave* slave = new ProcessorElementSlave(
            std::string("Slave_" + std::to_string(processorElementSlaves.size())).c_str(),
            slavePositions.at(processorElementSlaves.size()), masterPositions.at(processorElementSlaves.size()),
            *initialSlaveChar);
        ProcessorElementSlaveShell* slaveShell = new ProcessorElementSlaveShell(
            std::string("SlaveShell_" + std::to_string(processorElementSlaveShells.size())).c_str(),
