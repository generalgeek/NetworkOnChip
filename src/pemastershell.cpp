...
ProcessorElementMasterShell::ProcessorElementMasterShell(sc_module_name name, unsigned position,
    ...
    NoCDebug::printDebug(
        std::string("> MasterShell: M" + std::to_string(position) + "S" + std::to_string(slavePosition)),
        NoCDebug::Assembly);
...
void ProcessorElementMasterShell::_threadRun() {
    ...
    for (;;) {
        // Writing
        NoCDebug::printDebug(
            "M" + std::to_string(_position) + "S" + std::to_string(_slavePosition) + ": MShell <- Master",
            NoCDebug::NI);
        shellIn.read(send);
        std::vector<uint32_t> payload;
        payload.push_back(send);
        int payloadDst = _slavePosition;
        NoCDebug::printDebug("M" + std::to_string(_position) + "S" + std::to_string(_slavePosition) + ": MShell -> NIM",
                             NoCDebug::NI);
