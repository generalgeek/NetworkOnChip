...
ProcessorElementSlaveShell::ProcessorElementSlaveShell(sc_module_name name, unsigned position, unsigned masterPosition):
    sc_module(name), _position(position), _masterPosition(masterPosition) {
    NoCDebug::printDebug(
        std::string("> SlaveShell: S" + std::to_string(position) + "M" + std::to_string(masterPosition)),
        NoCDebug::Assembly);
...
void ProcessorElementSlaveShell::_threadRun() {
    ...
    for (;;) {
        ...
        int payloadSrc;
        NoCDebug::printDebug(
            "S" + std::to_string(_position) + "M" + std::to_string(_masterPosition) + ": SShell <- SKernel",
            NoCDebug::NI);
        receivePayload(payload, &payloadSrc);
        rec = payload.at(0);
        NoCDebug::printDebug(
            "S" + std::to_string(_position) + "M" + std::to_string(_masterPosition) + ": SShell -> Slave",
            NoCDebug::NI);
        shellOut.write(rec);
