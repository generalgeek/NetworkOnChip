...
void Kernel_ni::packetMaker() {
    ...
    for (;;) {
        ...
        if (debug >= 1)
            cout << "[" << this->name() << "::packetMaker] bufferPM_port0: " << std::hex << queueOut[0] << std::dec
                 << endl;
        ...
        for (i = 1; i <= burst; i++) { // Read next flits
            ...
            if (debug >= 1)
                cout << "[" << this->name() << "::packetMaker] bufferPM_port" << i << ": " << std::hex << queueOut[i]
                     << std::dec << endl;
        }
        ...
        for (i = 0; i <= burst; i++) {
            ...
            if (debug >= 1)
                cout << "[" << this->name() << "::packetMaker]:<<<flit" << i << " sent: [" << std::hex << (queueOut[i])
                     << std::dec << "]" << endl;
        }
    }
}
...
void Kernel_ni::packetDisassembly() {
    ...
    for (;;) {
        ...
        if (debug >= 1)
            cout << "[" << this->name() << "::packetDisassembly]: >>>flit" << 0 << " received: [" << std::hex
                 << queueIn[0] << std::dec << "]" << endl;
        ...
        for (i = 1; i <= burst; i++) {
            ...
            if (debug >= 1)
                cout << "[" << this->name() << "::packetDisassembly]: >>>flit" << i << " received: [" << std::hex
                     << queueIn[i] << std::dec << "]" << endl;
        }
