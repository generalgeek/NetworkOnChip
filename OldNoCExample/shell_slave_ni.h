...
SC_MODULE(ShellSlaveInterface) {
    ...
    sc_fifo_out<flit1_4t> cmd_port;
    //  sc_fifo_out<sc_uint<10> >
    // burst_port;

    sc_fifo_in<flit_t> rd_data_port;
    sc_fifo_in<flit1_4t> wr_resp_port;
    ...
    ShellSlaveInterface(sc_module_name _name, flit1_4t identify): sc_module(_name), identify(identify) {
        SC_THREAD(proc);
    }
