...
SC_MODULE(Slave) {
    sc_fifo_in<flit_t> wr_data_port, addr_port;
    sc_fifo_in<flit1_4t> cmd_port;

    sc_fifo_out<flit_t> rd_data_port;
    sc_fifo_out<flit1_4t> wr_resp_port;

    SC_HAS_PROCESS(Slave);
    // constructor
    Slave(sc_module_name _name, unsigned int identify): sc_module(_name), identify(identify) {
...
void Slave::test_slave() {
    ...
    for (;;) {
        ...
        if ((cmd = cmd_port.read()) & 8) { // CMD_SB, CMD_SH or CMD_SW
            ...
            if (debug >= 1)
                cout << "[" << this->name() << "]: Value: " << MEM[(int)address] << " written at address: " << address
                     << endl
                     << endl;
            ...
            if (debug >= 1)
                cout << "[" << this->name() << "]: Value: " << MEM[(int)address] << " read from address: " << address
                     << endl
                     << endl;
        }
