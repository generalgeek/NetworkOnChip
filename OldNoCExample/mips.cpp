...
void mips_v0::debug_decode() {
    ...
    if (debug >= 0)
        cout << "PC = " << dec << pc << endl;
    if (debug >= 0)
        cout << "opcode = 0x" << hex << opcode << " rs = 0x" << hex << rs << " rt = 0x" << hex << rt << " rd = 0x"
             << hex << rd << " shamt = 0x" << hex << shamt << " funct = 0x" << hex << funct << endl;
}
...
void mips_v0::execute() {
    while (pc < DATA_SEGMENT_START) {
        ...
        case ADDI:
            ...
            break;
        case ADDIU:
            breg[rt] = (uint32_t)breg[rs] + kte16;
            break;
        case SLTI:
            breg[rt] = breg[rs] < kte16;
            break;
        case SLTIU:
            breg[rt] = (uint32_t)breg[rs] < (uint32_t)kte16;
            break;
        case ANDI:
            breg[rt] = breg[rs] & (uint32_t)kte16;
            break;
        case ORI:
            breg[rt] = breg[rs] | (uint32_t)kte16;
            break;
        ...
        case LUI:
            ...
            break;
        case LB:
            breg[rt] = pshell->lb(breg[rs], kte16);
            break;
        case LBU:
            breg[rt] = pshell->lbu(breg[rs], kte16);
            break;
        case LH:
            breg[rt] = pshell->lh(breg[rs], kte16);
            break;
        case LW:
            breg[rt] = pshell->lw(breg[rs], kte16);
            break;
        case LHU:
            breg[rt] = pshell->lhu(breg[rs], kte16);
            break;
        case SB:
            pshell->sb(breg[rs], kte16, get_byte_0(breg[rt]));
            break;
        case SH:
            pshell->sh(breg[rs], kte16, (uint16_t)(breg[rt] & 0xFFFF));
            break;
        case SW:
            pshell->sw(breg[rs], kte16, breg[rt]);
            break;
        default:
            break;
        }
